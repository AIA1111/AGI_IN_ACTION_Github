...
def setup_logging():
    """Configure enhanced logging with comprehensive WebSocket diagnostics"""
    ...
    # File handler for detailed debugging logs
    detailed_formatter = logging.Formatter(
        '%(asctime)s.%(msecs)03d [%(levelname)s] %(funcName)s:%(lineno)d - %(message)s',
        datefmt='%H:%M:%S'
    )

    file_handler = logging.FileHandler('./logs/websocket_server_detailed.log', mode='w', encoding='utf-8')
    file_handler.setFormatter(detailed_formatter)
    file_handler.setLevel(logging.DEBUG)

    # Console handler for real-time monitoring in PyCharm
    ...
    # Create separate server logger for WebSocket operations
    server_logger = logging.getLogger('WebSocketServer')
    server_logger.setLevel(logging.DEBUG)
    server_logger.handlers.clear()

    # Server-specific file handler
    server_file_handler = logging.FileHandler('./logs/websocket_connections.log', mode='w', encoding='utf-8')
    ...
    server_console_handler.setFormatter(console_formatter)
    server_console_handler.setLevel(logging.INFO)
...
# Add this right after the setup_logging function
server_logger = logging.getLogger('WebSocketServer')
...
def log_connection_event(event_type: str, client_addr: str, additional_info: str = ""):
    """Log detailed connection events with timing"""
    timestamp = time.strftime('%H:%M:%S.%f')[:-3]
    connection_count = len(active_connections) if 'active_connections' in globals() else 0
    ...
    message = f"{emoji} {event_type}: {client_addr} | Active: {connection_count} | {additional_info}"
...
async def handle_websocket_connection(websocket, path, window):
    """Enhanced WebSocket handler with version 3 synchronous processing + version 4 diagnostics"""
    client_addr = f"{websocket.remote_address[0]}:{websocket.remote_address[1]}"
    connection_start_time = time.time()

    # Keep all our enhanced diagnostic logging from version 4
    ...
    try:
        # Keep all the connection management improvements from version 4
        ...
        def update_gui_log(message):
            """Thread-safe GUI log update - kept from version 4"""
            try:
        ...
        # Main message processing loop - this is where we revert to version 3 synchronous approach
        async for message in websocket:
            request_start_time = time.time()
            message_length = len(message)

            # Keep enhanced logging from version 4
            ...
            try:
                ...
                # REVERTED: Process TTS synchronously like version 3, but with version 4 logging
                if request_type == 'tts':
                    text = data.get('text', '')
                    ...
                    # Update GUI to show processing - kept from version 4
                    try:
                    ...
                    if pipeline:
                        try:
                            # CRITICAL: This is the version 3 synchronous processing that was working
                            audio_data = []
                            generator = pipeline(text, voice=voice)

                            # Process TTS synchronously in main loop - no async tasks
                            ...
                            if audio_data:
                                # Concatenate audio and prepare for HTTP delivery
                                full_audio = np.concatenate(audio_data)

                                # Save as FLAC for HTTP delivery - same as version 3
                                buffer = io.BytesIO()
                                sf.write(buffer, full_audio, 24000, format='FLAC')
                                audio_bytes = buffer.getvalue()
                                audio_size = len(audio_bytes)

                                # Create unique audio token for HTTP delivery - same as version 3
                                ...
                                # Save audio file for HTTP download - same as version 3
                                with open(temp_audio_path, 'wb') as f:
                                    f.write(audio_bytes)

                                # Send response with audio token - same as version 3
                                response = {
                                    'type': 'tts_response',
                                ...
                                # Enhanced logging from version 4, but after successful synchronous processing
                            ...
                            # Enhanced error handling from version 4
                            error_response = {
                                'type': 'error',
                ...
                # REVERTED: Process STT synchronously like version 3, but with version 4 logging
                ...
                # Handle heartbeat ping - kept from version 4 health check improvements
                    ...
                    # Unknown message type - enhanced logging from version 4
        ...
        # Enhanced connection lifecycle logging from version 4
        ...
        # Enhanced error logging from version 4
        connection_duration = time.time() - connection_start_time
        ...
        # Enhanced cleanup logging from version 4
...
def process_uploaded_audio_file(file_path, window=None):
    """Process uploaded audio file for STT - Direct file processing like Desktop STT buttons"""
    try:
        logger.info(f"[STT_FILE_PROCESS] Processing uploaded file: {file_path}")

        if not os.path.exists(file_path):
            logger.error(f"[STT_FILE_PROCESS] File not found: {file_path}")
            return None

        file_size = os.path.getsize(file_path)
        logger.info(f"[STT_FILE_PROCESS] File size: {file_size} bytes")

        if window:
            window['stt_status'].update("🌐 Processing uploaded STT file...")

        # Use exact same method as working Desktop STT buttons
        if whisper_model:
            try:
                # Read audio file directly (same as Desktop STT approach)
                import soundfile as sf
                ...
                logger.info(f"[STT_FILE_PROCESS] Audio loaded: {len(audio_data)} samples at {sample_rate}Hz")

                # Validate audio length
                min_samples = 8000  # 0.5 seconds at 16kHz
                if len(audio_data) < min_samples:
                    logger.warning(f"[STT_FILE_PROCESS] Audio too short: {len(audio_data)} samples")

                # Process with Whisper (same as Desktop STT)
                logger.info("[STT_FILE_PROCESS] Starting Whisper transcription...")
                result = whisper_model.transcribe(audio_data.flatten())

                text = result['text'].strip()
                language = result.get('language', 'unknown')

                logger.info(f"[STT_FILE_PROCESS] STT completed: '{text[:50]}...' (Language: {language})")

                # Update GUI if available
                if window:
                    window['stt_output'].update(text)
                    window['stt_language'].update(f"Language: {language}")
                    window['stt_status'].update("✅ Uploaded STT completed")
                ...
                logger.error(f"[STT_FILE_PROCESS] Whisper processing error: {e}")
                if window:
                    window['stt_status'].update(f"❌ STT Error: {str(e)[:50]}...")
                return None

        else:
            logger.error("[STT_FILE_PROCESS] Whisper model not loaded")
            if window:
                window['stt_status'].update("❌ Whisper model not available")
            return None

    except Exception as e:
        logger.error(f"[STT_FILE_PROCESS] File processing error: {e}")
        if window:
            window['stt_status'].update(f"❌ File processing error: {str(e)[:50]}...")
        return None
...
def main():
    ...
    while True:
        ...
        # STT Events
            ...
            # Run TTS generation in separate thread to prevent GUI freezing
            def run_tts_generation():
                generate_speech_kokoro(text, voice, save_path, filename_prefix, add_timestamp, window)

            thread = threading.Thread(target=run_tts_generation, daemon=True)
            thread.start()

        # STT Events - Modified to prevent GUI freezing
        elif event == 'stt_record5' and whisper_model and not is_recording:
            def run_stt_recording():
                record_audio_stt(5, window)

            thread = threading.Thread(target=run_stt_recording, daemon=True)
            thread.start()

        elif event == 'stt_record10' and whisper_model and not is_recording:
            def run_stt_recording():
                record_audio_stt(10, window)
            ...
            # Run microphone test in separate thread to prevent GUI freezing
            def run_mic_test():
                test_microphone()

            thread = threading.Thread(target=run_mic_test, daemon=True)
            thread.start()
