...
class WebSocketClient_TTSAndSTT:
    ...
    def send_tts_request(self, text: str, voice: str = None) -> Optional[bytes]:
        """Send TTS request - simple working version"""
        if not self.connected:
            self._log_message("Not connected - cannot send TTS request")
            return None

        # If no voice provided, load from config
        if voice is None:
            try:
                import json
                import os
                config_file = "BrowsingAgent_Config/tts_config.json"
                if os.path.exists(config_file):
                    with open(config_file, 'r') as f:
                        config = json.load(f)
                    voice = config.get('last_voice', 'af_heart')
                else:
                    voice = 'af_heart'  # Config doesn't exist yet
            except Exception:
                voice = 'af_heart'  # Any error - use safe default
    ...
    def send_stt_request_from_file(self, file_path: str) -> Optional[Dict[str, str]]:
        """Send STT request from audio file - Enhanced with logging"""
        try:
            self._log_message(f"[STT_FROM_FILE] Processing file: {file_path}")

            if not os.path.exists(file_path):
                self._log_message(f"[STT_FROM_FILE] File not found: {file_path}")
                return None

            # Read audio file and send to M3 server via WebSocket
            audio_data, sample_rate = sf.read(file_path)
            if len(audio_data.shape) > 1:
                audio_data = audio_data[:, 0]
            return self.send_stt_request(audio_data, sample_rate)  # ✅ CORRECT - sends to M3
        except Exception as e:
            self._log_message(f"[STT_FROM_FILE] Error: {e}")
            return None
